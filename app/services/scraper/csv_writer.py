...
def write_avalanche_forecast_to_csv(forecasts: List[VarsomAvalancheResponse]):
    rows = []
    for forecast in forecasts:
        rows.append(forecast)
    df = pd.DataFrame(rows)
    df.to_csv('avalanches.csv', index=False)


def write_weather_forecast_to_csv(forecasts: WeatherData, region: AvalancheRegion):
