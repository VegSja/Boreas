...
1. **Configure dbt profile** (`profiles.yml`):
```yaml
dbt_boreas:
  target: dev
  outputs:
...
# Run specific layer
dbt run --select models/2_silver
dbt run --select models/3_gold
...
dbt test --select models/2_silver
```
...
- Check model materialization strategy
- Consider incremental models for large tables
- Review query complexity

### Logs and Debugging
- Check `logs/dbt.log` for detailed error information
